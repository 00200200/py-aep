...
import pytest
from helpers import get_layer_from_json, load_expected
...
def test_position_reads_match_extendscript(sample: str) -> None:
    ...
    layer = parse_aep(SAMPLES_DIR / f"{sample}.aep").project.compositions[0].layers[0]
...
def test_expressibility_moves_to_the_followers_when_separated() -> None:
    layer = (
        parse_aep(SAMPLES_DIR / "transform_separated.aep")
        .project.compositions[0]
        .layers[0]
    )
...
def test_unseparated_keeps_expressibility_on_the_leader() -> None:
    layer = (
        parse_aep(SAMPLES_DIR / "transform_unseparated.aep")
        .project.compositions[0]
        .layers[0]
    )
